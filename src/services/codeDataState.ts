/**
 * 代码数据索引层（M40 巨型文件拆分：自 codeData.ts 迁出）：
 * 加载 public/data/ 下的数据库与旧版词库，构建内存索引（键/节/值类型/翻译词典/词库），
 * 供补全、翻译、高亮使用。查询函数在 codeDataLookup.ts，经本文件的 getter/访问器读索引。
 *
 * 数据来源：
 ...
 */
import { loadEnabledPluginData, type EnabledPluginData } from '../features/plugins/runtimeData'
...
/** 已启用插件的字段限定枚举解释；官方全局 value_zh 优先。 */
const pluginEnumExplanations = new Map<string, Map<string, string>>()
...
/** 把代码表模板键（{NUM}/{LANG}/#）编译为实际键匹配器（加载与查询共用）。 */
export function codeTemplateRegex(template: string): RegExp | undefined {
  let pattern = '^'
  ...
  if (!loaded) {
    ...
    loaded = (async () => {
      try {
        ...
        const pluginData: EnabledPluginData = await loadEnabledPluginDataFromStore()
        if (generation !== loadGeneration) return
        codes = nextCodes
        ...
        for (const [field, values] of Object.entries(pluginData.enumExplanations)) {
          const fieldMap = new Map<string, string>()
          for (const [val, zh] of Object.entries(values)) {
            if (val && zh) {
              fieldMap.set(val.toLowerCase(), zh)
              if (!valueZhDict.has(val.toLowerCase())) valueZhDict.set(val.toLowerCase(), zh)
              addValueAlias(String(zh), val)
            }
          }
          if (fieldMap.size > 0) pluginEnumExplanations.set(field.toLowerCase(), fieldMap)
        }
        ...
        for (const t of pluginData.translations) {
          if (t.en && t.zh) {
            enToZhDict.set(t.en.toLowerCase(), t.zh)
            addUniqueReverse(zhToEnDict, reverseAmbiguous, reverseOwners, t.zh, t.en, 0)
          }
        }
        ...
        aliasDict.clear()
        for (const a of pluginData.aliases) {
...
/** 获取指定字段的插件枚举解释；官方全局词典由 getValueZhDict 提供。 */
export function getPluginEnumExplanation(field: string, value: string): string | undefined {
  return pluginEnumExplanations.get(field.trim().toLowerCase())?.get(value.trim().toLowerCase())
}
...
// ── 查询层内部访问器（codeDataLookup.ts 专用；返回实时引用，不拷贝）────────

export function stateCodes(): readonly CodeInfo[] {
  return codes
}

export function stateSections(): readonly SectionInfo[] {
  return sections
}

export function stateValueTypes(): readonly ValueTypeInfo[] {
  return valueTypes
}

export function stateVocabulary(): readonly VocabularyItem[] {
  return vocabulary
}

export function stateDialectWords(): readonly VocabularyItem[] {
  return dialectWords
}

export function stateLogicBooleans(): readonly LogicBooleanInfo[] {
  return logicBooleans
}

export function stateOfficialUnits(): readonly OfficialUnitInfo[] {
  return officialUnits
}

export function stateGameVersions(): readonly GameVersionInfo[] {
  return gameVersions
}
