...
- **社区板块（本地预览）**：左侧「社区」入口进入独立工作区，推荐 / 关注 / 排行 / 我的四页签（精选轮换、创作者关注、排行维度切换、本地收藏）；当前展示**内置示例数据**，账号、在线发布、模组分享与下载在社区服务器上线后接入
...
```bash
npm run check      # 类型检查 + ESLint + 814 个单元测试，一键全跑
```

## 正在开发中

- **社区系统**：应用内已内置「社区」工作区（左侧入口，推荐 / 关注 / 排行 / 我的四页签），当前展示**本地示例数据**——无账号、无在线分享。账号、模组在线分享、模板市场与下载服务将在社区服务器上线后接入（社区后端规划见 `docs/BACKEND-PLAN.md`）。
